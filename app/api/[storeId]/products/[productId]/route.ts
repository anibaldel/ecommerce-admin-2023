...
export async function PATCH (
    ...
    try {
        ...
        if(!categoryId) {
            return new NextResponse("La categoria id es requerida", { status: 400 }) 
        }
        if(!sizeId) {
            return new NextResponse("El size id es requerido", { status: 400 }) 
        }
        if(!colorId) {
            return new NextResponse("El color id es requerido", { status: 400 }) 
        }
        if(!price) {
            return new NextResponse("El precio es requerido", { status: 400 }) 
        }
